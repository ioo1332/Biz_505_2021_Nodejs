// 전개연산자(spread,....)
// 배열과 객체의 값을 원본은 유지하면서
// 새로운 복제본을 만들때
// 일부를 추가,변경(update) 하고자 할때
// 코드를 간소하게 사용할수 있도록 하는 연산자

let 배열 = [1, 2, 3];

// 구상하기는 "배열"에 담긴 값을 새배열에 복제하고
// 새배열에 10을 추가하여 새로운 배열을 생성하고싶었는데
// 원본 배열이 변형되버렸다
let 새배열 = 배열;
새배열.push(10);
console.table(새배열);
// 원본 배열 변형 없이 추가
let 새배열4 = [];
배열.forEach((요소) => {
  새배열4.push(요소);
});
새배열4.push(40);
// 전개연산자 사용하기
// 전개연산자 원본인 배열에 모든 요소를 펼쳐놓고
// 거기에 새로운 값 20을 추가하고
// 값들만 새배열2에 복제한다
// 원본 배열은 손상 시키지않고 새로운 요소가 추가된 배열을 다시 만들수 있다
let 새배열2 = [...배열, 20];
console.table(새배열2);

let 새배열3 = [...배열, 30];
console.table(새배열3);

// 원본인 객체를 변형하지않고 원본의 내용을 모두 포함한
// 새로운 요소인 주소가 추가된 "객체복제" 객체를 만든다
let 객체 = { 이름: "홍길동", 전화: "1111" };
let 객체복제 = { ...객체, 주소: "서울시" };

let 숫자배열 = [100, 200, 300, 400, 500];
// 변수 백 이백 삼백 사백 오백 을 선언하고
// 숫자배열에 담긴 요소들을 순서대로
// 한개씩 선언된 변수에 담아서 초기화 하라
let [백, 이백, 삼백, 사백, 오백] = 숫자배열;

let 백1 = 100;
let 이백1 = 200;
let 삼백1 = 300;

console.log(백, 이백, 삼백, 사백, 오백);
// 배열의 요소를 만들고
let 나라들 = ["korea", "usa", "china", "japan", "english"];
// 배열의 요소를 초기값을 갖는 변수를 생성하기
let [한국, 미국, 중국, 일본, 영국] = 나라들;
console.log(한국, 미국, 중국, 일본, 영국);
